...
@section('content')
  ...
  <h3>Pending Death Claim List</h3>
        ...
        @foreach($all_claims as $key=>$single_data)
            <tr>
               ...
               <td>
                ...
                @if(auth()->user()->role==='manager')
                <td>
                    <a href="{{route('deathclaim.approved',$single_data->id)}}" class="btn btn-success">Approve</a>
                    <a href="{{route('deathclaim.reject',$single_data->id)}}" class="btn btn-success">Reject</a>
                </td>
                @endif
